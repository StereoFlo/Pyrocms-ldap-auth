...
class Adldap {
    ...
    */
    function __construct($options = array()){
		...
		if(count($options) == 0){
			...
			$this->_ci->config->load('auth/adldap', true);
    ...
    */
    ...
    public function search_user($query) {
		...
		$filter="(&(objectCategory=user)(|(cn=*$query*)(sAMAccountName=*$query*)(givenName=*$query*)(sn=*$query*)))";
		$justthese = array("ou", "sn", "givenname", "mail", "company", "sAMAccountName", "name", "title", "telephoneNumber");
		$sr = ldap_search($this->_conn, $this->_base_dn, $filter, $justthese);
    ...
    */
    ...
    /**
    * Delete an account (coumputer or user)
    * 
    * @param string $username The username to delete (please be careful here!)
    * @param bool $isGUID Is the username a GUID or a samAccountName
    * @return array
    */
    public function delete($type, $item, $isGUID = false) {
        if ($type == 'computer')
        {
            $info = $this->computer_info($item);
        }
        elseif ($type == 'user')
        {
            $info = $this->user_info($item, array("*"),$isGUID);
        }
    ...
    */
    ...
    /**
    * Move a any account to a different OU
    *
    * @param string $username is the type of an account: computer or user
    * @param string $existing_name is the existing name of the object to move
    * @param string $new_name is the new name of the object to move
    * @param string $new_location is the new OU of the object to move
    * @return bool
    */
    
    public function move_rename_account($type, $existing_name, $new_name = NULL, $new_location = NULL) {
    
        if ($type == NULL || $existing_name == NULL) {
            return ('type null');
        }
        ...
        if ($type=='computer') {
            $object_info = $this->computer_info($existing_name, $fields=array('distinguishedname','cn'));
        }
        else {
            $object_info = $this->info($existing_name, $fields = array('distinguishedname','cn'));
        }
        if ($object_info) {
            $dn = $object_info[0]['distinguishedname'][0];
            $cn = $object_info[0]['cn'][0];
            if ($new_name == NULL) {
                $rdn = 'CN=' . $cn;
            }
            else {
                $rdn = 'CN=' . trim($new_name);
            }
            if ($new_location == NULL) {
                $parent_ou = str_replace($cn, '', $dn);
            }
            else {
                $parent_ou = $new_location;
            }
            if ($dn && $rdn && $parent_ou) {
                $result = ldap_rename($this->_conn, $dn, $rdn, $new_location, true);
            }
        }
